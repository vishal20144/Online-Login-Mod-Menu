...
static monoString *FormatCount(int enemy, int bot) {
    char buf[128] = {0};
    sprintf(buf, "[PLAYER:%d BOT:%d]", enemy, bot);
    return U3DStr(buf);
}
static void SetColor(Color CurColor) {
    void (*_SetColor)(Color _Colors) = (void (*)(Color))getRealOffset(0x29DDE94);
    _SetColor(CurColor);
}
...
static void spofNick(void *players) {
    ...
    _spof_nick(players, U3DStr("OUSADO MODS CRACKED"));
}

static monoString *U3DStrFormat(float distance) {
    char buffer[128] = {0};
    sprintf(buffer, " [enemy distance: %.fm]", distance);
